...
async function test() {
    const { data, error } = await supabase.from('styles').select('id, styleNo, buyerId, buyerName, buyerPO, color, sizeWiseDetails, season').limit(1);
    if (error) {
        console.error("Styles schema error:", error.message);
    } else {
