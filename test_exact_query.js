...
async function test() {
    console.log("Fetching exact frontend query with limit 50...");
    const { data, error } = await supabase.from('styles').select('*').order('created_at', { ascending: false }).limit(50);
    
    if (error) {
        console.error("Error:", error);
    } else {
        console.log(`Fetched ${data?.length} styles.`);
    }
}
